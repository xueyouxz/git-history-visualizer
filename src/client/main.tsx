...
type Task = { id: string; phase: string; progress: number; message: string; recoverable?: boolean; repositoryPath?: string };
...
function App() {
  ...
  const start = async () => { try { setError(''); const next: Task = await api('/api/imports', { method: 'POST', body: JSON.stringify({ kind, source }) }); setTask(next); const timer = setInterval(async () => { const state = await api(`/api/imports/${next.id}`); setTask(state); if (state.phase === 'complete' || state.phase === 'error') clearInterval(timer); }, 300); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } };
