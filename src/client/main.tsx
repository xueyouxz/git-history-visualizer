...
function HistoryGraph() {
  const { commits, refs, topology, selectedOid, hoveredOid, semanticZoom, boxedOids, select: selectCommit, hover, setSemanticZoom, setBoxedOids } = useHistoryStore();
...
function App() {
  ...
  const { repositories, repositoryId, setRepositories, openRepository } = useHistoryStore();
  ...
  const loadRepositories = async () => {
    const next = await api<RepositorySummary[]>('/api/repositories'); setRepositories(next); if (!repositoryId && next[0]) openRepository(next[0].id); if (next.length) setShowImport(false);
  };
