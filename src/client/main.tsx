...
import { CHANGE_SIZE_LIMITS, COMMIT_CLASSIFICATION_VERSION, COMMIT_TYPES, type ChangeSizeFilter, type CommitType, type IndexedCommit, type RepositoryClassifications, type RepositoryIndex, type RepositoryRef, type RepositorySummary, type RepositoryTopology } from '../shared/history';
...
import { WorktreeControl } from './worktree-control';
...
function HistoryGraph() {
  const { commits, refs, topology, classifications, phaseAnalysis, phaseOverrides, selectedOid, hoveredOid, highlightedOids, contributorHighlightOids, aOid, bOid, semanticZoom, boxedOids, select: selectCommit, hover, setSemanticZoom, setBoxedOids } = useHistoryStore();
  ...
  const phaseStarts = phaseAnalysis ? [0, ...new Set(phaseAnalysis.boundaries.map(boundary => phaseOverrides[boundary.oid] ?? boundary.order).filter(order => order > 0 && order < (topology?.nodes.length ?? 0)))].sort((left, right) => left - right) : [];
  const phaseRegions = phaseStarts.map((start, index) => ({ start, end: phaseStarts[index + 1] ?? topology?.nodes.length ?? nodes.length, index }));
  ...
  return <section className="graph-panel" aria-label="提交拓扑">
    ...
    <svg ref={svgRef} className="dag" viewBox="0 0 1000 430" role="img" aria-labelledby="dag-title dag-description" onPointerDown={event => { if (event.shiftKey) { const point = pointer(event); setBoxStart(point); setBoxEnd(point); event.currentTarget.setPointerCapture(event.pointerId); } }} onPointerMove={event => { if (boxStart) setBoxEnd(pointer(event)); }} onPointerUp={finishBox}>
      ...
      <g transform={transform.toString()}>
        {phaseRegions.map(region => <g key={`${region.start}-${region.end}`} className={`phase-region phase-${region.index % 2}`}><rect x={12 + region.start * 138} y="8" width={Math.max(1, (region.end - region.start) * 138)} height={graphHeight - 16} /><text x={20 + region.start * 138} y="24">阶段 {region.index + 1}</text></g>)}
...
function Workspace({ api, session }: { api: Api; session: Session }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const parameters = historyFilterParameters(store);
